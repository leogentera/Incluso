	...
	public boolean execute(String action, JSONArray args, CallbackContext callbackContext)
		...
		if (action.equals("sayHello")){
			...
			try {
				...
				Context context=this.cordova.getActivity().getApplicationContext();
				Intent intent = context.getPackageManager().getLaunchIntentForPackage("com.revarts.JuegosIncluso");
                if (intent == null) {
                    intent = new Intent(Intent.ACTION_VIEW);
                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    intent.setData(Uri.parse("market://details?id=" + "com.revarts.JuegosIncluso"));
                    context.startActivity(intent);
                    JSONObject jsonObject= new JSONObject();
                    jsonObject.put("messageerror", Base64.encode("El juego no esta instalado".getBytes(), Base64.DEFAULT));
                    callbackContext.error(jsonObject);
                    return true;
                }
				intent.setFlags(0);
				//intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				intent.putExtra("game_arguments", args.getString(0));
				global.setCallbackContext(callbackContext);
