...
class progress_plugin extends external_api{
	...
	public static function get_global_progress($userid, $courseid){
		global $USER;
		global $DB;
		$response = array();
	...
	public static function get_finished_stages($userid, $courseid){
		global $USER;
		global $DB;
		$response = array();
		...
		try {
			...
			where  se.id=ger.sectionid
			and stage.courseid=$courseid
			and stage.courseid = activities.courseid
			and activities.courseid=stage.courseid
			and ger.courseid=se.course
			and ger.name = 'parent'
			and ger.value<>0
			and ger.value=stage.section
			and ger.sectionid=activities.sectionid
			and ger.courseid = activities.courseid
	...
	public static function get_all_stages($courseid){
		global $USER;
		global $DB;
		$response = array();
		...
		try {
			...
			$sql = "select gerStage.sectionid stageid, seStage.name stage
					...
					and gerStage.courseid=$courseid";
